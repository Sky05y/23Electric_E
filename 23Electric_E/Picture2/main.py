...
def crop_image(image,points):
    mask = np.zeros_like(image)         # 创建一个与图像相同大小的黑色掩码
    cv2.fillPoly(mask, [points], (255, 255, 255))
    cropped_image = cv2.bitwise_and(image, mask)    # 将掩码应用到图像上
    #提取多边形区域的边界框
    rect = cv2.boundingRect(points)
    #裁剪出多边形区域
    cropped_image = cropped_image[rect[1]:rect[1]+rect[3], rect[0]:rect[0]+rect[2]]
...
if __name__ == '__main__':
    ...
    #先找矩形 
    #返回红绿点坐标而不是图片
    p_red , p_green = find_circle.find_red_green(_img ,Location_Rect) #传入四点坐标
    if p_red != -1:
    ...
    while True:
        ...
        img_display = resize_picture(img_display , 50) #设定缩放比例，默认缩小为原来的50%
